...
| OP-7 | previsao-cripto: SerpAPI (mesma credencial do SEC-1) esgotada há 3 noites seguidas (18-20/07) — 28/28 previsões/noite com `input_degradado=1` (LLM analisa sem notícias); sem `NEWS_FALLBACK_PROVIDER` configurado | `OPEN_OPERATIONAL_GAP` | Padrão de log (retry-com-backoff todo dia, inclusive 1º ativo de dia UTC novo) sugere cota MENSAL/plano, não diária — inferência forte, não confirmada (log não grava status HTTP real, só tipo da exceção). Distinto do SEC-1 (aquele é vazamento da chave; este é esgotamento de cota da mesma chave). Correção pronta e NÃO aplicada (`NEWS_FALLBACK_PROVIDER=curated_rss`/`google_news_rss`, sem chave, já implementado): decisão de governança pendente — ligar dentro da H5 atual vs. abrir trial nova (H6), ver `previsao-cripto/HANDOFF.md` |
