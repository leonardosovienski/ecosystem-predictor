...
| RES-11 | lol-predictor H4 (2026-07-22): coorte prospectiva versionada, status cumulativo e avaliador determinístico foram implementados. Cotações legadas sem competição/proveniência não são reclassificadas como H4; dinheiro real continua bloqueado mesmo se o shadow passar. | `RESOLVED_AND_VERIFIED` | `src/h4_gate.py`, `scripts/market_shadow_status.py`, `scripts/evaluate_h4_gate.py`; suíte/CI locais. Amostra é uma limitação científica de tempo, não bug técnico. |
