...
| SCI-7 | Fase 1b via mercado: **parte LoL desbloqueada em 2026-07-20** com APIs públicas read-only do Polymarket (Gamma discovery + CLOB order book/histórico, sem autenticação), integradas localmente com provenance e enforcement PRE_EVENT. Polymarket é registrado como `prediction_market`, não bookmaker. A coleta prospectiva ainda não tem amostra madura, mas isso já não é bloqueio de fonte. A parte CS permanece sem integração verificada nesta rodada, fora do escopo exclusivo do lol-predictor | `PARTIALLY_RESOLVED_LOL` | LoL: fonte resolvida, iniciar shadow prospectivo sem trading; CS: continua dependência externa própria |
