...
| OP-4 | Backup/retenção/restore testado para os bancos SQLite/FeatureStore de cada consumidor (`audit/` OQ-040) | `PARTIALLY_RESOLVED` | F1: rebuild offline determinístico verificado. CS (2026-07-20): `src.backup_restore` cria snapshot consistente via API SQLite, inclui ratings/snapshots, manifesta SHA-256, verifica `integrity_check` e restaura somente em raiz nova; restore real confirmou 17.320 partidas. Segue aberto para os demais consumidores e para uma política humana comum de retenção/local externo. |
...
| SCI-7 | Fase 1b via mercado: Polymarket oferece APIs públicas read-only e cobertura de CS2/LoL. **LoL operacional desde 2026-07-20**: fallback DoH restrito, H4 pré-registrada e tarefa `lol-market-shadow` a cada 30 min (`LastTaskResult=0`); janela inicial 38 linhas, 32 elegíveis, 10 partidas, 0 maturadas, gate 50/30 dias/3 competições. **CS operacional desde 2026-07-20**: Gamma/CLOB read-only, fallback DoH restrito, identidade e formato exatos, enforcement PRE_EVENT, dedupe concorrente, descoberta automática em 48h e primeira janela com 15 cotações; nenhum caminho de trading. | `RESOLVED_SOURCE_AWAITING_SAMPLE` | LoL e CS: acumular amostra prospectiva sem analisar cedo nem fazer trading; passagem do tempo não pode ser acelerada por código. |
